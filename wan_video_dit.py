...
def flash_attention(q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, num_heads: int, compatibility_mode=False, attention_mask=None, return_KV=False):
    if attention_mask is not None:
        if not BLOCK_SPARSE_ATTN_AVAILABLE:
            raise RuntimeError(
                "block_sparse_attn is required when attention_mask is provided. "
                "Install Block-Sparse-Attention on a CUDA machine, or use LSWA/SDPA without block-sparse masks."
            )
        seqlen = q.shape[1]
        seqlen_kv = k.shape[1]
...
class SelfAttention(nn.Module):
    def __init__(self, dim: int, num_heads: int, eps: float = 1e-6,
                 lswa_spatial_window: Optional[Tuple[int, int]] = None,
                 lswa_temporal_window: int = 2,
                 lswa_query_chunk_size: int = 1024):
        super().__init__()
        self.dim = dim
        self.num_heads = num_heads
        ...
        # LSWA config (not in state_dict — does not affect weight loading)
        self.lswa_spatial_window = lswa_spatial_window  # (h_w, w_w) or None
        self.lswa_temporal_window = lswa_temporal_window
        self.lswa_query_chunk_size = int(lswa_query_chunk_size)
        self._lswa_mask_cache = {}  # {(H, W, h_w, w_w): bool tensor (L, L)}
    ...
    # ----------------------------------------------------------------
    @torch.no_grad()
    def _build_single_frame_mask(self, H: int, W: int, device: torch.device) -> torch.Tensor:
        """Build (L, L) bool spatial mask for one frame. Cached by (H, W, h_w, w_w)."""
        h_w, w_w = self.lswa_spatial_window
        cache_key = (H, W, h_w, w_w)
        if cache_key in self._lswa_mask_cache:
            cached = self._lswa_mask_cache[cache_key]
            if cached.device == device:
                return cached
            cached = cached.to(device)
            self._lswa_mask_cache[cache_key] = cached
            return cached
        ...
        # (L,1) vs (1,L) broadcast → (L, L)
        in_r = (idx.unsqueeze(0) // W >= (q_r.unsqueeze(1) - r_half).clamp(min=0)) & \
               (idx.unsqueeze(0) // W <= (q_r.unsqueeze(1) + r_half).clamp(max=H - 1))
        in_c = (idx.unsqueeze(0) % W  >= (q_c.unsqueeze(1) - c_half).clamp(min=0)) & \
               (idx.unsqueeze(0) % W  <= (q_c.unsqueeze(1) + c_half).clamp(max=W - 1))
        mask = in_r & in_c  # (L, L)

        self._lswa_mask_cache[cache_key] = mask
        return mask

    def _sdpa_attention(self, q: torch.Tensor, k: torch.Tensor, v: torch.Tensor,
                        attn_mask: torch.Tensor) -> torch.Tensor:
        """F.scaled_dot_product_attention with bool mask. q/k/v: (B, seq, D)."""
        B, L_q, D = q.shape
        K_L = k.shape[1]
        q = q.view(B, L_q, self.num_heads, self.head_dim).transpose(1, 2)
        k = k.view(B, K_L, self.num_heads, self.head_dim).transpose(1, 2)
        v = v.view(B, K_L, self.num_heads, self.head_dim).transpose(1, 2)
        out = F.scaled_dot_product_attention(q, k, v, attn_mask=attn_mask)
        return out.transpose(1, 2).contiguous().view(B, L_q, D)

    @torch.no_grad()
    def _lswa_offsets(self, device: torch.device) -> tuple[torch.Tensor, torch.Tensor]:
        h_w, w_w = self.lswa_spatial_window
        r_half, c_half = h_w // 2, w_w // 2
    ...
    def _local_spatial_attention(
        ...
        (B, H*W, D). The method materializes only one query chunk of local
        21x21xT neighborhoods at a time, avoiding the impossible full LxL mask
        at 1080x1920 training resolution.
        """
        B, L, D = q_frame.shape
        assert L == h * w, "Frame token length mismatch."
        qh = q_frame.view(B, L, self.num_heads, self.head_dim).permute(0, 2, 1, 3).contiguous()
        kh_frames = [
            k_frame.view(B, L, self.num_heads, self.head_dim).permute(0, 2, 1, 3).contiguous()
            for k_frame in k_context
        ]
        vh_frames = [
            v_frame.view(B, L, self.num_heads, self.head_dim).permute(0, 2, 1, 3).contiguous()
            for v_frame in v_context
        ]
        off_r, off_c = self._lswa_offsets(q_frame.device)
        n_spatial = int(off_r.numel())
        chunk_size = max(1, int(self.lswa_query_chunk_size))
        out = qh.new_empty(B, self.num_heads, L, self.head_dim)
        ...
        for start in range(0, L, chunk_size):
            ...
            for k_frame, v_frame in zip(kh_frames, vh_frames):
                gather_idx = neigh_idx.view(1, 1, end - start, n_spatial, 1).expand(
                    B, self.num_heads, end - start, n_spatial, self.head_dim
                )
                k_expand = k_frame.unsqueeze(2).expand(B, self.num_heads, end - start, L, self.head_dim)
                v_expand = v_frame.unsqueeze(2).expand(B, self.num_heads, end - start, L, self.head_dim)
    ...
    def _lswa_forward(self, q, k, v, f, h, w, is_stream, pre_cache_k, pre_cache_v):
        """LSWA path: frame loop with chunked local spatial neighborhoods."""
        B = q.shape[0]
        L = h * w  # tokens per frame
        ...
        for i in range(f):
            ...
            if len(context_k) > self.lswa_temporal_window:
                context_k = context_k[-self.lswa_temporal_window:]
                context_v = context_v[-self.lswa_temporal_window:]

            out_i = self._local_spatial_attention(q_i, context_k, context_v, h, w)
            out_frames.append(out_i)

        x = torch.cat(out_frames, dim=1)  # (B, f*L, D)
        return self.o(x), k_frames[-1], v_frames[-1]

    # ----------------------------------------------------------------
    # Block-Sparse path (original code, unchanged)
    # ----------------------------------------------------------------
    def _block_sparse_forward(self, q, k, v, B, f, h, w, D, local_num, topk,
                              kv_len, is_stream, pre_cache_k, pre_cache_v, local_range):
        win = (2, 8, 8)
        q = q.view(B, f, h, w, D)
        k = k.view(B, f, h, w, D)
        v = v.view(B, f, h, w, D)
    ...
    # ----------------------------------------------------------------
    # forward: routes to LSWA or Block-Sparse
    # ----------------------------------------------------------------
    def forward(self, x, freqs, f=None, h=None, w=None, local_num=None, topk=None,
        ...
        if self.lswa_spatial_window is not None:
            return self._lswa_forward(q, k, v, f, h, w, is_stream,
                                      pre_cache_k, pre_cache_v)
        else:
            return self._block_sparse_forward(q, k, v, B, f, h, w, D,
                                              local_num, topk, kv_len,
                                              is_stream, pre_cache_k, pre_cache_v,
                                              local_range)
...
class DiTBlock(nn.Module):
    def __init__(self, dim: int, num_heads: int, ffn_dim: int, eps: float = 1e-6,
        ...
        self.self_attn = SelfAttention(dim, num_heads, eps,
                                       lswa_spatial_window=lswa_spatial_window,
                                       lswa_temporal_window=lswa_temporal_window,
                                       lswa_query_chunk_size=lswa_query_chunk_size)
...
class WanModel(torch.nn.Module):
    def __init__(
        ...
        self.patch_size = patch_size
        self.lswa_spatial_window = lswa_spatial_window
    ...
    def forward(self,
        ...
        # Block-Sparse hyperparams (skipped when LSWA)
        if self.lswa_spatial_window is None:
            win = (2, 8, 8)
            seqlen = f // win[0]
            ...
            # LSWA mode: these are unused but set for signature compatibility
            local_num, topk, kv_len = None, None, None
        ...
        # blocks (is_stream=False for training; cache outputs ignored)
        for block_id, block in enumerate(self.blocks):
            ...
            if self.training and use_gradient_checkpointing:
                ...
                x, _, _ = block(x, context, t_mod, freqs, f, h, w, local_num, topk,
                                train_img, block_id, kv_len, is_full_block, False,
                                None, None)
